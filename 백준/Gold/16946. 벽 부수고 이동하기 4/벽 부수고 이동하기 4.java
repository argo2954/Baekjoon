...
public class Main {
    ...
    static void union(int r, int c){
        ...
        while(!queue.isEmpty()){
            ...
            for(int i=0; i<4; i++){
                int newR = state.r + dr[i];
                int newC = state.c + dc[i];

                if(isOut(newR, newC)) continue;     // 방문 불가(장외)
                if(MAP[newR][newC] == 1) continue;  // 방문 불가(벽)
                if(states[newR][newC].parent != null) continue; // 방문 불가(이전 탐색)

                queue.add(states[newR][newC]);
                list.add(states[newR][newC]);

                states[newR][newC].parent = root;
                size++;
            }
        }// 0 방문 끝
